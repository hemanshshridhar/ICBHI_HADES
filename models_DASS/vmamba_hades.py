...
class SS2Dv2:
    def __initv2__(
        ...
        **kwargs,
        ...
        self.blur_enable = kwargs.get("blur_enable", False)
        self.blur_kernel_size = kwargs.get("blur_kernel_size", 5)
        self.blur_sigma = kwargs.get("blur_sigma", 1.0)
        self.blur_strength = kwargs.get("blur_strength", 1.0)
        Linear = Linear2d if channel_first else nn.Linear
        ...
        self.total_filters = d_inner
        self.select_filters = 4 * math.ceil(d_inner//6)
        ...
        self.h_proj = nn.Linear(self.d_inner+self.d_inner, self.select_filters + self.total_filters//2, bias=bias, **factory_kwargs)
    ...
    def _gaussian_kernel1d(self,kernel_size, sigma, device, dtype):
        ax = torch.arange(kernel_size, device=device, dtype=torch.float32) - (kernel_size - 1) / 2.0
        g = torch.exp(-(ax**2) / (2.0 * sigma**2))
        g = g / (g.sum() + 1e-12)
        return g.to(dtype)

    def blur_separable(self, x, kernel_size=7, sigma=1.5):
        B, D, H, W = x.shape
        g = self._gaussian_kernel1d(kernel_size, sigma, x.device, x.dtype)  # (k,)
        w_h = g.view(1, 1, kernel_size, 1).expand(D, 1, kernel_size, 1).contiguous()
        w_w = g.view(1, 1, 1, kernel_size).expand(D, 1, 1, kernel_size).contiguous()
        pad = kernel_size // 2
        x = F.conv2d(x, w_h, padding=(pad, 0), groups=D)
        x = F.conv2d(x, w_w, padding=(0, pad), groups=D)
        return x
        ...
        **kwargs,
        ...
        if cascade2d:
            ...
            if no_einsum:
                ...
                dts = F.conv1d(dts.contiguous().view(B, -1, L), dt_projs_weight.view(K * D, -1, 1), groups=K) # (B,L,K,D)
            else:
...
class hades_layer(nn.Module):
    def __init__(self, blocks, downsample, blur_block_id=2):
        super().__init__()
        self.blocks = nn.ModuleList(blocks)
        self.downsample = downsample
        self.blur_block_id = blur_block_id
    ...
    def forward(self, x):
        ...
        for i, block in enumerate(self.blocks):
            x = block(x)                           # Sequential chain intact
            if i == self.blur_block_id:            # only block 2
                lb_loss  = block.op.lb_loss
                div_loss = block.op.div_loss

...
class Swin_FDWA(nn.Module):
    ...
    def forward(self, x, x_size):
        ...
        # Ensure C is divisible by 4 for 4 branches
        assert C % 4 == 0, "Channel dimension must be divisible by number of branches (4)"

        if self.shift_size > 0:
            qkv = qkv.view(3, B, H, W, C)  # [3, B, H, W, C]
            # Rearrange and split qkv along channel dim for 4 branches
            qkv = qkv.permute(1, 2, 3, 0, 4)  # [B, H, W, 3, C]
            qkv = qkv.chunk(4, dim=-1)        # tuple of 4 tensors, each [B, H, W, 3, C//4]
            qkv0, qkv1, qkv2, qkv3 = [t.permute(3, 0, 1, 2, 4) for t in qkv]  # each [3, B, H, W, C//4]
...
class FAT_Block(nn.Module):
    def __init__(self, trans_dim, head_dim, window_size, window_size_fm,drop_path, type='W',hyper=False):
        ...
        self.conv1_1 = nn.Conv2d(1, self.trans_dim, 1, 1, 0, bias=True)
        self.conv1_2 = nn.Conv2d(self.trans_dim, 1, 1, 1, 0, bias=True)
...
class ResidualBlockUpsample(nn.Module):
    ...
    def forward(self, x):
        identity = self.skip(self.upsample(x))
        out = self.relu(self.conv1(self.upsample(x)))
        out = self.conv2(out)
        out = self.relu(out + identity)
        return out        
# if __name__ == "__main__":
#     B, T, F = 2, 1024, 256
#     spec = torch.randn(B, T, F)

#     block = FAT_Block(trans_dim=256, head_dim=8, window_size=8,
#                   window_size_fm=16, drop_path=0.0, type='W')
#     out = block(spec)
#     print(out.shape)    
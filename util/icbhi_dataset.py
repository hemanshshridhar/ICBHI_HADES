...
class ICBHIDataset(Dataset):
    def __init__(self, train_flag, transform, args, print_flag=True, mean_std=False):
        data_folder = os.path.join(args.data_folder, 'audio_test_data')
        ...
        if args.model == 'audio_clap':
            cache_name = 'clap_'
        else:
            cache_name = ''

        cache_path = './data/{}training.pt'.format(cache_name) if self.train_flag else './data/{}test.pt'.format(cache_name)
        print('cache_name {} cache_path {}'.format(cache_name, cache_path))
        if not os.path.isfile(cache_path):
            ...
            meta_file = pd.read_csv(os.path.join(args.data_folder, 'metadata.txt'), names=['age', 'sex', 'adult_BMI', 'child_weight', 'child_height', 'chest_location'], delimiter= '\t')
            ...
            if self.train_flag:
                torch.save(self.audio_images, './data/{}training.pt'.format(cache_name))
                torch.save(self.class_nums, './data/class_nums.pt')
            else:
                torch.save(self.audio_images, './data/{}test.pt'.format(cache_name))
    
            # ==========================================================================
        else:
            if self.train_flag:
                self.audio_images = torch.load('./data/{}training.pt'.format(cache_name), weights_only=False)
                self.class_nums = torch.load('./data/class_nums.pt', weights_only=False)
            else:
                self.audio_images = torch.load('./data/{}test.pt'.format(cache_name), weights_only=False)
